...
public class CalcVisitor extends CymbolBaseVisitor<Integer> {
  @Override
  public Integer visitBlock(CymbolParser.BlockContext ctx) {
    ctx.stat().stream()
        .filter(stat -> stat instanceof CymbolParser.ExprStatContext)
        .forEach(exprStat -> System.out.println(((CymbolParser.ExprStatContext) exprStat).expr().getText() + " = " + visit(exprStat)));
    return null;
  }

  @Override
  public Integer visitExprStat(CymbolParser.ExprStatContext ctx) {
